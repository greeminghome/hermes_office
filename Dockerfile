...
WORKDIR /app
ARG VITE_OFFICE_BRAND_NAME="Hermes Office"
ARG VITE_OFFICE_BRAND_SHORT_NAME="Hermes"
ARG VITE_OFFICE_BRAND_DESCRIPTION="Self-hosted AI team workspace"
ENV VITE_OFFICE_BRAND_NAME=$VITE_OFFICE_BRAND_NAME \
    VITE_OFFICE_BRAND_SHORT_NAME=$VITE_OFFICE_BRAND_SHORT_NAME \
    VITE_OFFICE_BRAND_DESCRIPTION=$VITE_OFFICE_BRAND_DESCRIPTION
COPY package*.json ./
RUN npm ci
COPY . .
RUN npm run build
