...
_browser_cdp_override_var = contextvars.ContextVar("browser_cdp_override", default=None)
_browser_router_override_var = contextvars.ContextVar("browser_router_override", default=None)
...
def _browser_route_for_profile_index(index: int) -> tuple[str, str]:
    """Resolve a validated profile index without requiring gateway env state."""
    ...
    router_port = int(os.environ.get("HERMES_PROFILE_CDP_PROXY_BASE_PORT") or 9400) + index
    return (
...
def _browser_session_owner_path(session_key: str) -> Path:
    root = Path(
        os.environ.get("HERMES_BROWSER_SESSION_OWNER_DIR")
        or "/opt/data/browser-session-owners"
    )
...
def register_browser_session_profile(session_key: str, profile: str) -> tuple[str, str]:
    """Persist browser ownership before a new session's first tool call.

    Hermes may execute browser tools in another process.  ContextVars and
    in-memory route maps cannot cross that boundary, and the canonical DB row
    is intentionally created lazily.  This small, atomic ownership record
    closes that first-turn race without sharing browser state across profiles.
    """
    key = str(session_key or "").strip()
    name = str(profile or "").strip()
    ...
    try:
        ...
        try:
            if path.read_text(encoding="utf-8") == payload:
                with _browser_routes_lock:
                    _browser_profile_routes_by_session[key] = route
                return route
        except OSError:
            pass
        temporary = path.with_name(
        ...
        logger.warning(
            "Could not persist browser ownership session=%s profile=%s: %s",
            key,
            name,
            exc,
        )
    ...
    with _browser_routes_lock:
        _browser_profile_routes_by_session[key] = route
    logger.info(
        "Registered durable browser ownership session=%s profile=%s route=%s",
        key,
        name,
        route[1],
    )
...
def _persisted_browser_session_route(session_key: str) -> tuple[str, str]:
    """Read an exact cross-process browser ownership record, if present."""
    try:
        payload = json.loads(
            _browser_session_owner_path(session_key).read_text(encoding="utf-8")
        )
    except (OSError, ValueError, TypeError):
        return "", ""
    if not isinstance(payload, dict) or payload.get("sessionKey") != session_key:
...
def set_browser_route_override(
    cdp_url: str, router_url: str = "", session_key: str = ""
) -> tuple:
    """Bind one Hermes session to its profile-specific persistent browser."""
    key = str(session_key or "").strip()
    if key:
        with _browser_routes_lock:
            _browser_routes_by_session[key] = (
                (cdp_url or "").strip(),
                (router_url or "").strip(),
            )
    return (
        _browser_cdp_override_var.set((cdp_url or "").strip() or None),
        _browser_router_override_var.set((router_url or "").strip() or None),
        key,
    )


def reset_browser_route_override(tokens: tuple) -> None:
    if not tokens:
        return
    _browser_cdp_override_var.reset(tokens[0])
    _browser_router_override_var.reset(tokens[1])


def _registered_browser_route(task_id: Optional[str] = None) -> tuple[str, str]:
    """Return a durable per-session route even across tool worker threads.

    ContextVars do not automatically cross every executor boundary used by
    tool calls. The gateway therefore registers the route by durable Hermes
    session key before starting a turn, and browser operations resolve it with
    their explicit task id.
    """
    ...
    if not key:
        return "", ""
    with _browser_routes_lock:
        return _browser_routes_by_session.get(key, ("", ""))


def _profile_browser_route_for_session(task_id: Optional[str] = None) -> tuple[str, str]:
    """Resolve a durable session to the persistent browser owned by its profile.

    Browser tools can execute in a worker whose ContextVars do not contain the
    gateway's profile metadata.  The durable session row is the authoritative
    ownership record, so use a read-only lookup across the configured profile
    databases before accepting a default route registered by such a worker.

    Only a unique match is accepted.  An absent or duplicated session fails
    closed to the caller's existing route instead of exposing another
    profile's browser context.
    """
    key = str(task_id or "").strip()
    if not key:
        try:
    ...
    with _browser_routes_lock:
        cached = _browser_profile_routes_by_session.get(key)
    if cached:
        return cached

    persisted = _persisted_browser_session_route(key)
    if persisted[0]:
        with _browser_routes_lock:
        ...
        try:
            ...
            try:
                row = connection.execute(
                    "SELECT 1 FROM sessions WHERE id = ? LIMIT 1", (key,)
                ).fetchone()
            finally:
                connection.close()
            if row:
            ...
            logger.debug(
                "Profile state lookup failed for session=%s profile=%s: %s",
                key,
                profile,
                exc,
            )
    ...
    if len(matches) != 1:
        if len(matches) > 1:
            logger.error(
                "Refusing ambiguous browser ownership for session=%s profiles=%s",
                key,
                [profile for _, profile in matches],
            )
        return "", ""

    _, profile = matches[0]
    route = _browser_route_for_profile(profile)
    with _browser_routes_lock:
        _browser_profile_routes_by_session[key] = route
    logger.info(
        "Resolved durable browser ownership session=%s profile=%s route=%s",
        key,
        profile,
        route[1],
    )
...
def _get_cdp_override(task_id: Optional[str] = None) -> str:
    ...
    endpoint.
    """
    profile_override, _ = _profile_browser_route_for_session(task_id)
    if profile_override:
        return _resolve_cdp_override(profile_override)

    registered_override, _ = _registered_browser_route(task_id)
    if registered_override:
        return _resolve_cdp_override(registered_override)

    context_override = _browser_cdp_override_var.get()
    if context_override:
        return _resolve_cdp_override(context_override)
...
def _create_cdp_session(task_id: str, cdp_url: str) -> Dict[str, str]:
    ...
    _, profile_router = _profile_browser_route_for_session(task_id)
    _, registered_router = _registered_browser_route(task_id)
    router = (
        profile_router
        or registered_router
        or _browser_router_override_var.get()
        or os.environ.get("HERMES_BROWSER_SESSION_ROUTER", "")
    ).strip().rstrip("/")
    routed_cdp_url = cdp_url
    if router:
        parsed = urlparse(router)
        scheme = "wss" if parsed.scheme == "https" else "ws"
        routed_cdp_url = urlunparse(
            (scheme, parsed.netloc, f"/session/{quote(task_id, safe='')}", "", "", "")
        )
    logger.info("Created CDP browser session %s → %s for task %s",
                session_name, routed_cdp_url, task_id)
    return {
        "session_name": session_name,
        "bb_session_id": None,
        "cdp_url": routed_cdp_url,
        "session_router_url": router or None,
        "features": {"cdp_override": True, "session_routed": bool(router)},
    }


def browser_session_target(task_id: str) -> Dict[str, Any]:
    """Return the exact Chrome target assigned to a Hermes task session."""
    session_key = _last_session_key(task_id)
    with _cleanup_lock:
        session_info = dict(_active_sessions.get(session_key) or {})
    router = str(
        session_info.get("session_router_url")
        or _profile_browser_route_for_session(session_key)[1]
        or _registered_browser_route(session_key)[1]
        or os.environ.get("HERMES_BROWSER_SESSION_ROUTER", "")
    ).rstrip("/")
    if not router:
        return {}
    response = requests.get(
        f"{router}/__session_target?{urlencode({'session': session_key})}",
        timeout=3,
    )
    response.raise_for_status()
    payload = response.json()
...
def release_browser_session(task_id: str) -> bool:
    ...
    This is reserved for a Hermes session that was explicitly closed, deleted,
    or archived.  Ordinary agent-browser inactivity cleanup intentionally does
    ...
    registered_route = _registered_browser_route(session_key)
    router = str(
        session_info.get("session_router_url")
        or persisted_route[1]
        or registered_route[1]
        or os.environ.get("HERMES_BROWSER_SESSION_ROUTER", "")
    ).rstrip("/")
    ...
    if router:
        try:
            ...
            response.raise_for_status()
            disposed = bool((response.json() or {}).get("disposed"))
        except Exception as exc:
            logger.warning("Could not dispose Chrome context for session %s: %s", session_key, exc)
...
def _camofox_eval(expression: str, task_id: Optional[str] = None) -> str:
    ...
    try:
        tab_info = _ensure_tab(_effective_task_id(task_id))
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    # A CDP override points at the persistent Chrome instance shown in Live
    # Screen. agent-browser's annotated capture paints its numbered boxes into
    # that shared page, so the operator sees the model-only SOM overlay and it
    # can remain visible when a capture is interrupted. The accessibility
    # snapshot still supplies stable @eN refs, while the shared screen stays a
    # clean, directly controllable browser view.
    if annotate and _get_cdp_override():
        logger.info("browser_vision: suppressing annotation on shared CDP session")
        annotate = False
