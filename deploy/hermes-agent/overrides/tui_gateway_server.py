...
def _profile_browser_route(profile: str | None) -> tuple[str, str]:
    """Resolve the persistent Chrome and session router owned by a profile."""
    name = (profile or "").strip()
    if not name:
        cdp_port = int(os.environ.get("HERMES_BROWSER_CDP_PORT") or 9222)
...
def _bind_live_session_profile(
    session: dict, profile: str | None, profile_home: Path | None
) -> bool:
    """Bind a resumed live session to its requested profile, failing closed.

    Older gateway sessions may predate profile-aware resume and therefore have
    no ``profile`` value even though the durable session belongs to a named
    profile. Repairing that missing metadata is safe. Reusing a live session
    already owned by a different named profile is not safe, so callers must
    reject that resume instead of exposing another profile's browser/session.
    """
    requested = (profile or "").strip() or None
    current = (session.get("profile") or "").strip() or None
    ...
    if requested:
        ...
        _register_session_browser_profile(session)
    return True


def _session_browser_profile(session: dict) -> str | None:
    """Return the profile that owns this session's persistent browser."""
...
def _register_session_browser_profile(session: dict) -> tuple[str, str]:
    """Publish browser ownership for workers before lazy DB persistence."""
    profile = _session_browser_profile(session)
    session_key = str(session.get("session_key") or "").strip()
    if not profile or not session_key:
        return "", ""
    try:
        from tools.browser_tool import register_browser_session_profile

        return register_browser_session_profile(session_key, profile)
    except Exception as exc:
        logger.warning(
            "Could not register browser ownership session=%s profile=%s: %s",
            session_key,
            profile,
            exc,
        )
...
def _office_coordination_metadata(name: str, args: dict) -> dict | None:
    """Expose only explicit Agent Office routing markers from delegate_task."""
    if name != "delegate_task" or not isinstance(args, dict):
        return None
    try:
    ...
    assignments = list(dict.fromkeys(
        match.strip()
        for match in re.findall(r"\[OFFICE_ASSIGN\s+profile=([^\]]+)\]", raw, flags=re.IGNORECASE)
        if match.strip()
    ))
    meeting_match = re.search(r"\[OFFICE_MEETING\s+participants=([^\]]+)\]", raw, flags=re.IGNORECASE)
    meeting_participants = []
    if meeting_match:
        meeting_participants = list(dict.fromkeys(
            item.strip() for item in meeting_match.group(1).split(",") if item.strip()
        ))
    if not assignments and not meeting_participants:
        return None
    return {
        "assignments": assignments,
        "meeting": bool(meeting_participants),
...
def _on_tool_start(sid: str, tool_call_id: str, name: str, args: dict):
    ...
    if _tool_progress_enabled(sid):
        ...
        office_coordination = _office_coordination_metadata(name, args)
        if office_coordination:
            payload["office_coordination"] = office_coordination
...
def _on_tool_complete(sid: str, tool_call_id: str, name: str, args: dict, result: str):
    ...
    if str(name or "").startswith("browser_"):
        try:
            from tools.browser_tool import browser_session_target

            task_id = str((session or {}).get("session_key") or sid)
            target = browser_session_target(task_id)
            if target.get("targetId"):
                _emit(
                    ...
                    sid,
                    {
                        ...
                        "browser_session_id": str(target.get("sessionId") or task_id),
                        "url": str(target.get("url") or ""),
                        "title": str(target.get("title") or ""),
                    },
                )
        except Exception:
            logger.debug("Could not publish browser.target for %s", sid, exc_info=True)
...
def _(rid, params: dict) -> dict:
    ...
    with _sessions_lock:
        ...
        _register_session_cwd(_sessions[sid])
        _register_session_browser_profile(_sessions[sid])
...
def _(rid, params: dict) -> dict:
    ...
    with _session_resume_lock:
        ...
        if live is not None:
            sid, session = live
            if not _bind_live_session_profile(session, profile, profile_home):
                return _err(rid, 4091, "session belongs to a different profile")
    ...
    with _session_resume_lock:
        ...
        if live is not None:
            ...
            if not _bind_live_session_profile(other_session, profile, profile_home):
                return _err(rid, 4091, "session belongs to a different profile")
            payload = _live_session_payload(
                other_sid,
                other_session,
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        release_browser_session(target)
    except Exception as exc:
        logger.warning("Could not release browser resources for deleted session %s: %s", target, exc)
    ...
    def run():
        ...
        try:
            ...
            from tools.browser_tool import (
                register_browser_session_profile,
                set_browser_route_override,
            )

            browser_profile = _session_browser_profile(session)
            if browser_profile:
                register_browser_session_profile(
                    session["session_key"], browser_profile
                )
            browser_cdp_url, browser_router_url = _profile_browser_route(browser_profile)
            browser_route_tokens = set_browser_route_override(
                browser_cdp_url,
                browser_router_url,
                session["session_key"],
            )
            ...
            if home_token is not None:
                reset_hermes_home_override(home_token)
            if browser_route_tokens is not None:
                from tools.browser_tool import reset_browser_route_override

                reset_browser_route_override(browser_route_tokens)
